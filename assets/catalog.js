...
(() => {
  ...
  const fields = ['resource_type', 'discipline', 'llm_use'];
  ...
  const records = rows.map(row => ({
    ...
    discipline: JSON.parse(row.dataset.discipline),
    llm_use: JSON.parse(row.dataset.llmUse),
    text: '',
  }));
  ...
  const legacyTopics = {
    '#section-articles': 'Research Articles',
    '#section-discussion-articles': 'Discussion Articles',
    '#section-tutorials': 'Tutorials on Using LLMs',
    '#section-useful-llm-documentation': 'LLM Documentation',
    '#section-use-case-examples': 'Use Case Examples',
  };
  function applyHash() {
